...
use BibleGet\Controls\StyleBar;
use BibleGet\Controls\FontSelect;
use BibleGet\Controls\TextAlign;
...
class Customize {
	...
	private static function initialize_options() {
		...
		foreach ( self::$bget_properties->options as $option => $array ) {
			if ( ! isset( self::$bget_options[ $option ] ) ) {
				self::$bget_options[ $option ] = $array['default'];
			}
		}
	}
	...
	public static function init() {
		...
		self::$bibleget_style_settings->{'BGET[PARAGRAPHSTYLES_LINEHEIGHT]'}->choices = [
			'1.0'  => __( 'single', 'bibleget-io' ),
			'1.15' => '1.15',
			'1.5'  => '1½',
			'2.0'  => __( 'double', 'bibleget-io' ),
		];
		...
		self::$bibleget_style_settings->{'BGET[PARAGRAPHSTYLES_WIDTH]'}->choices     = [
			'min'  => 10,
			'max'  => 100,
			'step' => 1,
		];
		...
		self::$bibleget_style_settings->{'BGET[PARAGRAPHSTYLES_BORDERSTYLE]'}->choices     = [];
		...
		self::$bibleget_style_settings->{'BGET[PARAGRAPHSTYLES_MARGINLEFTRIGHTUNIT]'}->choices     = [
			'px'   => 'px',
			'%'    => '%',
			'auto' => 'auto',
		];
		...
		$bibleget_styles_general->FONT_SIZE_UNIT->choices     = [
			'px'      => 'px',
			'em'      => 'em',
			'pt'      => 'pt',
			'inherit' => 'inherit',
		...
		$bibleget_style_sizes_arr   = [
			'min'  => 4,
			'max'  => 28,
			'step' => 1,
		];
		$bibleget_style_choices_arr = [
		...
		foreach ( $bibleget_styles_general as $i => $styleobj ) {
			...
			if ( $styleobj->controltype === 'range' ) {
				...
				self::$bibleget_style_settings->{'BGET[VERSIONSTYLES_' . $o . ']'}->settings = [
			...
			if ( $styleobj->controltype === 'range' ) {
				...
				self::$bibleget_style_settings->{'BGET[BOOKCHAPTERSTYLES_' . $o . ']'}->settings = [
			...
			if ( $styleobj->controltype === 'range' ) {
				...
				self::$bibleget_style_settings->{'BGET[VERSENUMBERSTYLES_' . $o . ']'}->settings = [
			...
			if ( $styleobj->controltype === 'range' ) {
				...
				self::$bibleget_style_settings->{'BGET[VERSETEXTSTYLES_' . $o . ']'}->settings = [
	...
	public static function add_panels_and_sections( $wp_customize ) {
	...
	public static function register( $wp_customize ) {
		...
		if ( false === self::$bget_properties instanceof Properties || false === property_exists( self::$bget_properties, 'OPTIONS' ) ) {
			self::initialize_options();
		}
		self::add_panels_and_sections( $wp_customize );
		...
		foreach ( self::$bibleget_style_settings as $style_setting => $style_setting_obj ) {
			...
			if ( strpos( $style_setting, 'FONTSTYLE' ) ) {
				...
				switch ( $style_setting_obj->controltype ) {
					case 'color':            // no break.
					case 'textalign':        // no break.
					case 'fontselect':
						self::add_custom_control(
							$wp_customize,
							$style_setting,
							$style_setting_obj,
							$bibleget_style_settings_cc,
							$style_setting_obj->controltype
						);
						break;
					case 'select':
						$ctl_atts = [
	...
	private static function add_custom_control( $wp_customize, $style_setting, $style_setting_obj, &$bibleget_style_settings_cc, $type ) {
		$options = [
			'label'    => $style_setting_obj->title,
			'settings' => $style_setting,
			'priority' => $bibleget_style_settings_cc++,
	...
	public static function build_customizer_stylesheet() {
		$is_googlefont = false;
		$fontfamily    = self::$bget_options['PARAGRAPHSTYLES_FONTFAMILY'];
		if ( ! empty( $fontfamily ) ) {
		...
		if ( $is_googlefont && ! empty( $fontfamily ) ) {
			...
			self::generate_options_css( '.bibleQuote.results', 'font-family', self::$bget_options['PARAGRAPHSTYLES_FONTFAMILY'] );
		}
		echo PHP_EOL;
		echo '.bibleQuote.results p { margin: 0; }';
		echo PHP_EOL;
		self::generate_options_css( '.bibleQuote.results', 'border-width', self::$bget_options['PARAGRAPHSTYLES_BORDERWIDTH'], '', 'px' );
		echo PHP_EOL;
		self::generate_options_css( '.bibleQuote.results', 'border-radius', self::$bget_options['PARAGRAPHSTYLES_BORDERRADIUS'], '', 'px' );
		echo PHP_EOL;
		self::generate_options_css( '.bibleQuote.results', 'border-color', self::$bget_options['PARAGRAPHSTYLES_BORDERCOLOR'], '', '' );
		echo PHP_EOL;
		self::generate_options_css( '.bibleQuote.results', 'border-style', BGET::CSSRULE['BORDERSTYLE'][ self::$bget_options['PARAGRAPHSTYLES_BORDERSTYLE'] ], '', '' );
		echo PHP_EOL;
		self::generate_options_css( '.bibleQuote.results', 'background-color', self::$bget_options['PARAGRAPHSTYLES_BACKGROUNDCOLOR'], '', '' );
		echo PHP_EOL;
		self::generate_options_css( '.bibleQuote.results', 'width', self::$bget_options['PARAGRAPHSTYLES_WIDTH'], '', '%' );
		echo PHP_EOL;

		$parStylesMarginTopBottom     = self::$bget_options['PARAGRAPHSTYLES_MARGINTOPBOTTOM'];
		$parStylesMarginLeftRight     = self::$bget_options['PARAGRAPHSTYLES_MARGINLEFTRIGHT'];
		$parStylesMarginLeftRightUnit = self::$bget_options['PARAGRAPHSTYLES_MARGINLEFTRIGHTUNIT'];
		$cssrule                      = sprintf(
			'%s { %s: %s %s; }',
			'.bibleQuote.results',
			'margin',
		...
		$parStylesPaddingTopBottom = self::$bget_options['PARAGRAPHSTYLES_PADDINGTOPBOTTOM'];
		$parStylesPaddingLeftRight = self::$bget_options['PARAGRAPHSTYLES_PADDINGLEFTRIGHT'];
		$cssrule                   = sprintf(
			'%s { %s: %s %s; }',
			'.bibleQuote.results',
			'padding',
		...
		self::generate_options_css( '.bibleQuote.results p.versesParagraph', 'text-align', BGET::CSSRULE['ALIGN'][ self::$bget_options['PARAGRAPHSTYLES_PARAGRAPHALIGN'] ] );
		echo PHP_EOL;
		self::generate_options_css( '.bibleQuote.results p.bibleVersion', 'color', self::$bget_options['VERSIONSTYLES_TEXTCOLOR'] );
		echo PHP_EOL;
		self::generate_options_css( '.bibleQuote.results .bookChapter', 'color', self::$bget_options['BOOKCHAPTERSTYLES_TEXTCOLOR'] );
		echo PHP_EOL;
		self::generate_options_css( '.bibleQuote.results p.versesParagraph', 'color', self::$bget_options['VERSETEXTSTYLES_TEXTCOLOR'] );
		echo PHP_EOL;
		self::generate_options_css( '.bibleQuote.results p.versesParagraph span.verseNum', 'color', self::$bget_options['VERSENUMBERSTYLES_TEXTCOLOR'] );
		echo PHP_EOL;
		...
		foreach ( $fontsizerules as $fontsizerule => $css_selector ) {
			$fontSize     = self::$bget_options[ $fontsizerule ];
			$fontSizeUnit = self::$bget_options[ $fontsizeunits[ $i++ ] ];
			if ( $fontSizeUnit === 'em' ) {
				$fontSize /= 10;
			}
			$cssrule = '';
		...
		foreach ( $fontstylerules as $fontstylerule => $css_selector ) {
			...
			$bold          = self::$bget_options[ $fontstylerule . 'BOLD' ];
			$italic        = self::$bget_options[ $fontstylerule . 'ITALIC' ];
			$underline     = self::$bget_options[ $fontstylerule . 'UNDERLINE' ];
			$strikethrough = self::$bget_options[ $fontstylerule . 'STRIKETHROUGH' ];
			// $fval = array();
			// if (!empty($mod)) {
			// $fval = explode(',', $mod);
			...
			if ( $fontstylerule === 'VERSENUMBERSTYLES_' ) {
				switch ( self::$bget_options['VERSENUMBERSTYLES_VALIGN'] ) {
			...
			/*
				...
				*/
			printf( '%s { %s }', $css_selector, $cssrule );
			echo PHP_EOL;
			// }
			// unset($fval);
		}
		...
		self::generate_options_css( '.bibleQuote.results p.versesParagraph', 'line-height', self::$bget_options['PARAGRAPHSTYLES_LINEHEIGHT'] . 'em' );
		// $linespacing_verses = self::$bget_options['PARAGRAPHSTYLES_LINEHEIGHT'].'em';
		// $poetic_linespacing = (self::$bget_options['PARAGRAPHSTYLES_LINEHEIGHT'] + 1.0).'em';
		$fontsize_versenumber = self::$bget_options['VERSENUMBERSTYLES_FONTSIZE'];
		if ( self::$bget_options['VERSENUMBERSTYLES_FONTSIZEUNIT'] === 'em' ) {
			$fontsize_versenumber /= 10;
		}
		$fontsize_versenumber .= self::$bget_options['VERSENUMBERSTYLES_FONTSIZEUNIT'];
		...
		switch ( intval( self::$bget_options['LAYOUTPREFS_BIBLEVERSIONALIGNMENT'] ) ) {
		...
		switch ( intval( self::$bget_options['LAYOUTPREFS_BOOKCHAPTERALIGNMENT'] ) ) {
			case BGET::ALIGN['CENTER']:
	...
	public static function header_output() {
		...
		self::build_customizer_stylesheet();
	}
	 ...
	 * @param bool   $echoback Optional. Whether to print directly to the page (default: true).
