		...
		if(preg_match_all("/<div class=\"errors\" id=\"errors\">.*?<\/div>/s", $output, $matches)){
			...
			if($error_rows != null && $error_rows->length > 0){
				...
				foreach($error_rows as $error_row) {
					$errormessage = getElementsByClass($error_row,'td','errMessageVal');
					$errs[] = "BIBLEGET SERVER ERROR: " . "<span style=\"color:Red;\">" . $errormessage[0]->nodeValue . "</span><span style=\"color:DarkBlue;\">(". current_page_url() .")</span>." . "<br />" . "<span style=\"color:Gray;font-style:italic;\">" . __("If this error continues, please notify the BibleGet plugin creator at:") . " <a target=\"_blank\" href=\"mailto:bibleget.io@gmail.com?subject=BibleGet+Server+Error&body=" . urlencode("The Wordpress Plugin is receiving this error message from the BibleGet Server:" . "\n\n" . $errormessage[0]->nodeValue . "\n\nKind regards,\n\n")."\">bibleget.io@gmail.com</a>" . "</span>";
				}
			}
	...
	foreach( $queries as $key => $value ) {
		...
		if($key === 0) {
			if(! preg_match( "/^[1-3]{0,1}((\p{L}\p{M}*)+)/", $thisquery )) {
				...
				$notices[] = "BIBLEGET PLUGIN ERROR: " . sprintf( __( "The first query <%s> in the querystring <%s> must start with a valid book indicator!", "bibleget-io" ), $thisquery, implode( ";", $queries ) ) . " (". current_page_url() .")";
				continue;
			}
		}
...
function checkQuery($thisquery, $indexes, $thisbook = "") {
	...
	if(preg_match( "/^([1-3]{0,1}((\p{L}\p{M}*)+))/", $thisquery, $res )) {
		...
		if(! preg_match( "/^[1-3]{0,1}((\p{L}\p{M}*)+)[1-9][0-9]{0,2}/", $thisquery ) || preg_match_all( "/^[1-3]{0,1}((\p{L}\p{M}*)+)/", $thisquery, $dummy ) != preg_match_all( "/^[1-3]{0,1}((\p{L}\p{M}*)+)[1-9][0-9]{0,2}/", $thisquery, $dummy )) {
			$errs[] = "BIBLEGET ERROR: " . $errorMessages[1] . " (". current_page_url() .")";
			update_option( 'bibleget_error_admin_notices', $errs );
			return false;
		}
		...
		if($validBookIndex != - 1) {
			...
			if(strpos( $thisquery, "." )) {
				if(! strpos( $thisquery, "," ) || strpos( $thisquery, "," ) > strpos( $thisquery, "." )) {
					...
					$errs[] = "BIBLEGET ERROR: malformed query <" . $thisquery . ">: " . $errorMessages[3] . " (". current_page_url() .")";
					update_option( 'bibleget_error_admin_notices', $errs );
					return false;
				}
				if(substr_count( $thisquery, "," ) > substr_count( $thisquery, "." )) {
					$errs[] = "BIBLEGET ERROR: malformed query <" . $thisquery . ">: " . $errorMessages[0] . " (". current_page_url() .")";
					update_option( 'bibleget_error_admin_notices', $errs );
					return false;
				}
				...
				if(preg_match_all( "/(?<![0-9])(?=([1-9][0-9]{0,2}\.[1-9][0-9]{0,2}))/", $thisquery, $dummy ) != substr_count( $thisquery, "." )) {
					...
					$errs[] = "BIBLEGET ERROR: malformed query <" . $thisquery . ">: " . $errorMessages[4] . " (". current_page_url() .")";
					update_option( 'bibleget_error_admin_notices', $errs );
					return false;
				}
				if(preg_match_all( "/(?<![0-9])(?=([1-9][0-9]{0,2}\.[1-9][0-9]{0,2}))/", $thisquery, $dummy )) {
					foreach( $dummy[1] as $match ) {
						...
						if(intval( $ints[0] ) >= intval( $ints[1] )) {
							...
							$errs[] = "BIBLEGET ERROR: malformed query <" . $thisquery . ">: " . $str . " (". current_page_url() .")";
							update_option( 'bibleget_error_admin_notices', $errs );
							return false;
						}
					}
			...
			if(strpos( $thisquery, "," )) {
				if(preg_match_all( "/[1-9][0-9]{0,2}\,[1-9][0-9]{0,2}/", $thisquery, $dummy ) != substr_count( $thisquery, "," )) {
					...
					$errs[] = "BIBLEGET ERROR: malformed query <" . $thisquery . ">: " . $errorMessages[5] . " (". current_page_url() .")";
					update_option( 'bibleget_error_admin_notices', $errs );
					return false;
				} else {
					if(preg_match_all( "/([1-9][0-9]{0,2})\,/", $thisquery, $matches )) {
						...
						foreach( $matches[1] as $match ) {
							foreach( $indexes as $jkey => $jindex ) {
								...
								if($match > $chapter_limit) {
									...
									$errs[] = "BIBLEGET ERROR: " . sprintf( $msg, $match, $thisbook, $jkey, $chapter_limit ) . " (". current_page_url() .")";
									update_option( 'bibleget_error_admin_notices', $errs );
									return false;
								}
							}
						...
						if($commacount > 1) {
							if(! strpos( $thisquery, '-' )) {
								/* translators: 'commas', 'dots', and 'dashes' refer to the bible citation notation; in some notations(such as english notation) colons are used instead of commas, and commas are used instead of dots */
								$errs[] = "BIBLEGET ERROR: " . __( "You cannot have more than one comma and not have a dash!", "bibleget-io" ) . " <" . $thisquery . ">" . " (". current_page_url() .")";
								update_option( 'bibleget_error_admin_notices', $errs );
								return false;
							}
							$parts = explode( "-", $thisquery );
							if(count( $parts ) != 2) {
								$errs[] = "BIBLEGET ERROR: " . __( "You seem to have a malformed querystring, there should be only one dash.", "bibleget-io" ) . " <" . $thisquery . ">" . " (". current_page_url() .")";
								update_option( 'bibleget_error_admin_notices', $errs );
								return false;
							}
							foreach( $parts as $part ) {
								...
								foreach( $indexes as $jkey => $jindex ) {
									...
									if($pp[1] > $verselimit) {
										...
										$errs[] = "BIBLEGET ERROR: " . sprintf( $msg, $pp[1], $thisbook, $pp[0], $jkey, $verselimit ) . " (". current_page_url() .")";
										update_option( 'bibleget_error_admin_notices', $errs );
										return false;
									}
								}
								...
								if(preg_match_all( "/[,\.][1-9][0-9]{0,2}\-([1-9][0-9]{0,2})/", $thisquery, $matches )) {
									...
									foreach( $indexes as $jkey => $jindex ) {
										...
										if($highverse > $verselimit) {
											/* translators: the expressions <%1$d>, <%2$s>, <%3$d>, <%4$s> and %5$d must be left as is, they will be substituted dynamically by values in the script. See http://php.net/sprintf. */
											$msg = __( 'A verse in the query is out of bounds: there is no verse <%1$d> in the book <%2$s> at chapter <%3$d> in the requested version <%4$s>, the last possible verse is <%5$d>', "bibleget-io" );
											$errs[] = "BIBLEGET ERROR: " . sprintf( $msg, $highverse, $thisbook, $parts[0], $jkey, $verselimit ) . " (". current_page_url() .")";
											update_option( 'bibleget_error_admin_notices', $errs );
											return false;
										}
									}
				...
				foreach( $chapters as $zchapter ) {
					foreach( $indexes as $jkey => $jindex ) {
						...
						if(intval( $zchapter ) > $chapter_limit) {
							/* translators: the expressions <%1$d>, <%2$s>, <%3$s>, and <%4$d> must be left as is, they will be substituted dynamically by values in the script. See http://php.net/sprintf. */
							$msg = __( 'A chapter in the query is out of bounds: there is no chapter <%1$d> in the book <%2$s> in the requested version <%3$s>, the last possible chapter is <%4$d>', "bibleget-io" );
							$errs[] = "BIBLEGET ERROR: " . sprintf( $msg, $zchapter, $thisbook, $jkey, $chapter_limit ) . " (". current_page_url() .")";
							update_option( 'bibleget_error_admin_notices', $errs );
							return false;
						}
					}
			...
			if(strpos( $thisquery, "-" )) {
				if(preg_match_all( "/[1-9][0-9]{0,2}\-[1-9][0-9]{0,2}/", $thisquery, $dummy ) != substr_count( $thisquery, "-" )) {
					...
					$errs[] = "BIBLEGET ERROR: malformed query <" . $thisquery . ">: " . $errorMessages[6] . " (". current_page_url() .")";
					update_option( 'bibleget_error_admin_notices', $errs );
					return false;
				}
				if(preg_match( "/\-[1-9][0-9]{0,2}\,/", $thisquery ) &&(! preg_match( "/\,[1-9][0-9]{0,2}\-/", $thisquery ) || preg_match_all( "/(?=\,[1-9][0-9]{0,2}\-)/", $thisquery, $dummy ) > preg_match_all( "/(?=\-[1-9][0-9]{0,2}\,)/", $thisquery, $dummy ))) {
					...
					$errs[] = "BIBLEGET ERROR: malformed query <" . $thisquery . ">: " . $errorMessages[7] . " (". current_page_url() .")";
					update_option( 'bibleget_error_admin_notices', $errs );
					return false;
				}
				if(substr_count( $thisquery, "-" ) > 1 &&(! strpos( $thisquery, "." ) ||(substr_count( $thisquery, "-" ) - 1 > substr_count( $thisquery, "." )))) {
					...
					$errs[] = "BIBLEGET ERROR: malformed query <" . $thisquery . ">: " . $errorMessages[8] . " (". current_page_url() .")";
					update_option( 'bibleget_error_admin_notices', $errs );
					return false;
				}
				...
				if(preg_match( "/([1-9][0-9]{0,2}\,[1-9][0-9]{0,2}\-[1-9][0-9]{0,2})/", $thisquery, $matchA )) {
					...
					if(preg_match( "/([1-9][0-9]{0,2}\,[1-9][0-9]{0,2}\-[1-9][0-9]{0,2}\,[1-9][0-9]{0,2})/", $thisquery, $matchB )) {
						...
						if($matchesB_LEFT[0] >= $matchesB_RIGHT[0]) {
							...
							$errs[] = "BIBLEGET ERROR: malformed query <" . $thisquery . ">: " . sprintf( __( "Chapters must be consecutive. Instead the first chapter indicator <%s> is greater than or equal to the second chapter indicator <%s> in the expression <%s>" ), $matchesB_LEFT[0], $matchesB_RIGHT[0], $matchB[1] ) . " (". current_page_url() .")";
							update_option( 'bibleget_error_admin_notices', $errs );
							return false;
						}
						...
						if($matchesA[0] >= $matchesA[1]) {
							/* translators: do not change the placeholders <%s>, they will be substituted dynamically by values in the script. See http://php.net/sprintf. */
							$errs[] = "BIBLEGET ERROR: malformed query <" . $thisquery . ">: " . sprintf( __( "Verses in the same chapter must be consecutive. Instead verse <%s> is greater than verse <%s> in the expression <%s>" ), $matchesA[0], $matchesA[1], $matchA[1] ) . " (". current_page_url() .")";
							update_option( 'bibleget_error_admin_notices', $errs );
							return false;
						}
					}
				...
				if(preg_match_all( "/\.([1-9][0-9]{0,2}\-[1-9][0-9]{0,2})/", $thisquery, $matches )) {
					foreach( $matches[1] as $match ) {
						...
						if($ints[0] >= $ints[1]) {
							/* translators: do not change the placeholders <%s>, they will be substituted dynamically by values in the script. See http://php.net/sprintf. */
							$errs[] = "BIBLEGET ERROR: malformed query <" . $thisquery . ">: " . sprintf( __( "Verses concatenated by a dash must be consecutive, instead <%s> is greater than or equal to <%s> in the expression <%s>" ), $ints[0], $ints[1], $match ) . " (". current_page_url() .")";
							update_option( 'bibleget_error_admin_notices', $errs );
							return false;
						}
					}
			...
			$errs[] = "BIBLEGET ERROR: " . $errorMessages[2] . " <" . $thisquery . ">" . " (". current_page_url() .")";
			update_option( 'bibleget_error_admin_notices', $errs );
			return false;
		}
	} else {
		if(! preg_match( "/^[1-9][0-9]{0,2}/", $thisquery )) {
			$errs[] = "BIBLEGET ERROR: " . $errorMessages[10] . " <" . $thisquery . ">" . " (". current_page_url() .")";
			update_option( 'bibleget_error_admin_notices', $errs );
			return false;
		}
	}
...
function getMetaData($request) {
	...
	if(curl_errno( $ch )) {
		...
		$notices[] = "BIBLEGET PLUGIN ERROR: " . sprintf( __( "There was a problem communicating with the BibleGet server. <a href=\"%s\" title=\"update metadata now\">Metadata needs to be manually updated</a>." ), $optionsurl ) . " (". current_page_url() .")";
		update_option( 'bibleget_error_admin_notices', $notices );
		return false;
	} else {
		...
		if($info["http_code"] != 200) {
			$optionsurl = admin_url( "options-general.php?page=bibleget-settings-admin" );
			/* translators: do not change the placeholders or the html markup, though you can translate the anchor title */
			$notices[] = "BIBLEGET PLUGIN ERROR: " . sprintf( __( "There may have been a problem communicating with the BibleGet server. <a href=\"%s\" title=\"update metadata now\">Metadata needs to be manually updated</a>." ), $optionsurl ) . " (". current_page_url() .")";
			update_option( 'bibleget_error_admin_notices', $notices );
			return false;
		}
	}
